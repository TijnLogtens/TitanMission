    ...
    public double[] update(ArrayList<CelestialBody> planets, double dt) {
		...
		for (Object object : planets) {
			if(object instanceof CelestialBody) {
                ...
                double distance = Math.pow((this.x+this.parent.getX() - (planet.getX()+planet.getParent().getX())),2) + 
                    Math.pow((this.y+this.parent.getY() - (planet.getY() + planet.getParent().getY())),2) + 
                    Math.pow((this.z+this.parent.getZ() - (planet.getZ()+planet.getParent().getZ())),2);
				double dist = Math.sqrt(distance);
				double temp = bigG * planet.getMass() / Math.pow(dist,3);

				// Euler acceleration -- ~First Order
				first[0] = temp * ((planet.getX()+planet.getParent().getX()) - this.x-this.parent.getX());
				first[1] = temp * ((planet.getY()+planet.getParent().getY()) - this.y-this.parent.getY());
				first[2] = temp * ((planet.getZ()+planet.getParent().getZ()) - this.z-this.parent.getZ());

				// ~Second Order after a half timestep
				double[] temp_vel = step(this.vx, this.vy, this.vz, first, 0.5 * dt);
				double[] temp_position = step(this.x, this.y, this.z, temp_vel, 0.5 * dt);
				second[0] = ((planet.getX()+planet.getParent().getX()) - temp_position[0]-this.parent.getX()) * temp;
				second[1] = ((planet.getY()+planet.getParent().getY()) - temp_position[1]-this.parent.getY()) * temp;
				second[2] = ((planet.getZ()+planet.getParent().getZ()) - temp_position[2]-this.parent.getZ()) * temp;

				// ~Third Order after a half timestep
				temp_vel = step(this.vx, this.vy, this.vz, second, 0.5 * dt);
				temp_position = step(this.x, this.y, this.z, temp_vel, 0.5 * dt);
				third[0] = ((planet.getX()+planet.getParent().getX()) - temp_position[0]-this.parent.getX()) * temp;
				third[1] = ((planet.getY()+planet.getParent().getY()) - temp_position[1]-this.parent.getY()) * temp;
				third[2] = ((planet.getZ()+planet.getParent().getZ()) - temp_position[2]-this.parent.getZ()) * temp;

				// ~Fourth Order after 1 timestep in the future using
				temp_vel = step(this.vx, this.vy, this.vz, third, dt);
				temp_position = step(this.x, this.y, this.z, temp_vel, dt);
				fourth[0] = ((planet.getX()+planet.getParent().getX()) - temp_position[0]-this.parent.getX()) * temp;
				fourth[1] = ((planet.getY()+planet.getParent().getY()) - temp_position[1]-this.parent.getY()) * temp;
				fourth[2] = ((planet.getZ()+planet.getParent().getZ()) - temp_position[2]-this.parent.getZ()) * temp;
