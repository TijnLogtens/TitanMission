...
public class Controller {
    ...
    private final static double MAXIMUM_WIND_EARTH = 100;
    ...
    public void updateTitanWind(double dt) {
        ...
        do {
            ...
            resultingWind = Math.sqrt((verticalWind + something * dt) * (verticalWind + something * dt)
                    + (horizontalWind + somethingElse * dt) * (horizontalWind + somethingElse * dt));
    ...
    /*
     * Format: 
     * verticalWind:horizontalWind:1 or 0 
     * 1 refers to successful landing 0 refers to unsuccessful landing
     */
    public double ReadFromFile() {
        ...
        try {
            ...
            do {
                ...
                if ((Math.abs(horizontalWind - Double.parseDouble(details[0])) < EPSILON)
                        && (Math.abs(verticalWind - Double.parseDouble(details[1])) < EPSILON)) {
    ...
    public double PD_ControllerXTitan(double dt, double x_coord, double mass) {
        return controller.PD_ControllerXTitan(dt, x_coord, mass);
    }

    public double PD_ControllerYTitan(double dt, double x_coord, double mass) {
        return controller.PD_ControllerYTitan(dt, x_coord, mass);
    }
